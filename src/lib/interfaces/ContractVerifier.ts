...
class ContractVerifier {
  ...
  verifyHashCodeEqualsConsistency(): void {
    if(this.testObjects.length === 0)
      this.warningWriter('No object was given to the Verifier. HashCode consistency with equals could not be tested.');
    ...
    for(const x of this.testObjects) {
      for(const y of this.testObjects) {
        if(x.equals(y)) {
          ...
          if(x.hashCode() !== y.hashCode()) throw new VerificationError('hashCode', 'equals consistency', [x, y]);
        }
      }
    }

    if(!atLeastOneEquals)
      this.warningWriter('Among the objects no equality was found, thus the HashCode consistency with equals could not be tested.');
