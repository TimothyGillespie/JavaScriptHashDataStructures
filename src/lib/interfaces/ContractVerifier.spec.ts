...
describe('ContractVerifier', () => {
	...
	describe('total equals verification', () => {
		...
		it('consistency failing deterministically failing', () => {
			const example = [
				new DeterministicallyInconsistentEquals(1),
				new DeterministicallyInconsistentEquals(2),
				new DeterministicallyInconsistentEquals(3),
			];
		...
		it('consistency failing probabilistically failing', () => {
			const example = [
				new ProbabilisticallyInconsistentEquals(1),
				new ProbabilisticallyInconsistentEquals(2),
				new ProbabilisticallyInconsistentEquals(3),
			];
			expect(() => new ContractVerifier(example).verifyEqualsContract()).toThrow(VerificationError);
		});
	});
	...
	describe('total hashCode verification', () => {
		...
		it('can determine that it is deterministically generally inconsistent', () => {
			const example = [
				new DeterministicallyInconsistentHashCode(1),
				new DeterministicallyInconsistentHashCode(2),
				new DeterministicallyInconsistentHashCode(3),
			];
		...
		it('can determine that it is probabilistically generally inconsistent', () => {
			const example = [
				new ProbabilisticallyInconsistentHashCode(1),
				new ProbabilisticallyInconsistentHashCode(2),
				new ProbabilisticallyInconsistentHashCode(3),
			];
			expect(() => new ContractVerifier(example).verifyHashCodeContract()).toThrow(VerificationError);
		});
		...
		it('can determine that it is equals inconsistent', () => {
			const example = [
				new InconsistentWithEqualsHashCode(1),
				new InconsistentWithEqualsHashCode(2),
				new InconsistentWithEqualsHashCode(3),
			];
			expect(() => new ContractVerifier(example).verifyHashCodeContract()).toThrow(VerificationError);
		});
	});
	...
	describe('singular hashCode methods', () => {
		describe('general consistency', () => {
			it('can determine that it is consistent', () => {
				const example = [new ValidHashable(1), new ValidHashable(2), new ValidHashable(3)];
				expect(() => new ContractVerifier(example).verifyHashCodeGeneralConsistency()).not.toThrow(
					VerificationError,
				);
			});
			...
			it('can determine that it is deterministically inconsistent', () => {
				...
				expect(() => new ContractVerifier(example).verifyHashCodeGeneralConsistency()).toThrow(
					VerificationError,
				);
			});
		...
		describe('equals consistency', () => {
			it('can determine that it is consistent', () => {
				const example = [new ValidHashable(1), new ValidHashable(2), new ValidHashable(3)];
				expect(() => new ContractVerifier(example).verifyHashCodeEqualsConsistency()).not.toThrow(
					VerificationError,
				);
			});
			...
			it('can determine that it is inconsistent', () => {
				...
				expect(() => new ContractVerifier(example).verifyHashCodeEqualsConsistency()).toThrow(
					VerificationError,
				);
			});
		});
		...
		describe('Warnings', () => {
			...
			it('equals reflexive does not warn when one example is given', () => {
				expect(() => new ContractVerifier([new ValidHashable(3)]).verifyEqualsReflexive()).not.toThrow(
					NotExtensiveExampleError,
				);
			});
			...
			it('equals symmetric when one example', () => {
				expect(() => new ContractVerifier([new ValidHashable(1)]).verifyEqualsSymmetric()).toThrow(
					NotExtensiveExampleError,
				);
			});
			...
			it('equals symmetric does not warn when two example are given', () => {
				expect(() =>
					new ContractVerifier([new ValidHashable(3), new ValidHashable(4)]).verifyEqualsSymmetric(),
				).not.toThrow(NotExtensiveExampleError);
			});
			...
			it('equals transitive when one example is given', () => {
				expect(() => new ContractVerifier([new ValidHashable(1)]).verifyEqualsTransitive()).toThrow(
					NotExtensiveExampleError,
				);
			});
			...
			it('equals transitive when two examples are given', () => {
				expect(() =>
					new ContractVerifier([new ValidHashable(1), new ValidHashable(2)]).verifyEqualsTransitive(),
				).toThrow(NotExtensiveExampleError);
			});
			...
			it('hashCode general consistent when no example', () => {
				expect(() => new ContractVerifier().verifyHashCodeGeneralConsistency()).toThrow(
					NotExtensiveExampleError,
				);
			});
			...
			it('hashCode general consistent does not warn when one example is given', () => {
				expect(() =>
					new ContractVerifier([new ValidHashable(5)]).verifyHashCodeGeneralConsistency(),
				).not.toThrow(NotExtensiveExampleError);
			});

			it('hashCode equals consistent when no example', () => {
				expect(() => new ContractVerifier().verifyHashCodeEqualsConsistency()).toThrow(
					NotExtensiveExampleError,
				);
			});
			...
			it('hashCode equals consistent does warn when no equal pair is found', () => {
				expect(() =>
					new ContractVerifier([new NotReflexive(5), new NotReflexive(5)]).verifyHashCodeEqualsConsistency(),
				).toThrow(NotExtensiveExampleError);
			});

			it('hashCode equals consistent does not warn when at least one pair is equal', () => {
				expect(() =>
					new ContractVerifier([new NotReflexive(5), new NotReflexive(6)]).verifyHashCodeEqualsConsistency(),
				).not.toThrow(NotExtensiveExampleError);
			});
		});
	});
